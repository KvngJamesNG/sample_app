...
# Defines the matching rules for Guard. guard :minitest, all_on_start: false do
  watch(%r{^test/(.*)/?(.*)_test\.rb$})
  watch('test/test_helper.rb') { 'test' } watch('config/routes.rb') { interface_tests } watch(%r{app/views/layouts/*}) { interface_tests } watch(%r{^app/models/(.*?)\.rb$}) do |matches|
    ...
    end watch('app/views/layouts/application.html.erb') do
    ...
    end watch(%r{app/views/users/*}) do
    ...
    'test/integration/following_test.rb'] end
    end
# Returns the integration tests corresponding to the given resource.def integration_tests(resource = :all) if resource == :all
